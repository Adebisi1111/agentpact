...
class AgentPact(gl.Contract):
    ...
    def submit_proof(
        ...
        # Verify signature (worker signed the proof hash)
        expected_message = f"proof:{agreement_id}:{proof_hash}:{nonce}"
        # In production, we'd use ecrecover here
        ...
        if is_valid:
            ...
            # Transfer payment if escrow has funds
            # This would use gl.transaction to send ETH
        else:
    ...
    def cancel_agreement(self, agreement_id: str) -> bool:
        ...
        # Refund calculation: unused ticks * payment_per_tick
        remaining = agreement.total_ticks - agreement.paid_ticks
        refund_amount = remaining * agreement.payment_per_tick
        
        # In production: transfer refund_amount back to hiree
